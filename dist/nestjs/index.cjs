...
function collectRoutes(srcPath, tsconfigPath, existingProject) {
  const project = existingProject ?? new tsMorph.Project({
...
function extractDecoratorStringArg(decorator) {
  ...
  const first = args[0];
  if (tsMorph.Node.isStringLiteral(first)) return first.getLiteralValue();
  return void 0;
}
  ...
  for (const dec of decorators) {
    if (dec.getName() === "UseGuards") {
      for (const arg of dec.getArguments()) {
        if (tsMorph.Node.isIdentifier(arg)) guards.push(arg.getText());
      }
    }
...
function mergePresets(presets, userContracts, userRules) {
  ...
  const contracts$1 = contracts.mergeContracts(...presetContracts, userContracts);
  const rules = {};
  for (const preset of presets) {
    if (preset.rules) Object.assign(rules, preset.rules);
  }
  if (userRules) Object.assign(rules, userRules);
  return { contracts: contracts$1, rules };
}
...
exports.nestjsDefaults = nestjsDefaults;
